...
public class NhanVienRepositoryImpl implements NhanVienRepository{
    ...
    public List<NhanVienViewModel> getList(int trangThai) {
        ...
        String sql = "SELECT  MaNhanVien,TenNhanVien ,GioiTinh ,NgaySinh,SoDienThoai,Email,DiaChi,MatKhau,TrangThai FROM NhanVien WHERE TrangThai LIKE ?";
        try {
    ...
    public boolean add(NhanVien nv) {
        String sql = "INSERT INTO NhanVien(TenNhanVien,GioiTinh,NgaySinh,SoDienThoai,Email,DiaChi,TrangThai)VALUES (?,?,?,?,?,?,?)";
        int check = 0;
        try {
            check = DBConnect.Update(sql, nv.getTenNhanVien(), nv.getGioiTinh(), nv.getNgaySinh(), nv.getSoDienThoai(), nv.getEmail(), nv.getDiaChi(),nv.getTrangThai());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return check > 0;
    ...
    public boolean update(String maNV, NhanVien nv) {
        String sql = "UPDATE NhanVien SET TenNhanVien = ?,GioiTinh = ?,NgaySinh = ?,SoDienThoai = ?,Email = ?,DiaChi = ?, TrangThai = ? WHERE MaNhanVien LIKE ?";
        int check = 0;
        try {
            check = DBConnect.Update(sql, nv.getTenNhanVien(), nv.getGioiTinh(), nv.getNgaySinh(), nv.getSoDienThoai(), nv.getEmail(), nv.getDiaChi(),nv.getTrangThai(), maNV);
